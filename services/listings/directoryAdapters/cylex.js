...
function parseResults(html) {
  const results = [];
  const pattern = /<div[^>]*class="[^"]*result[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/gi;
  let m;
  while ((m = pattern.exec(html)) !== null) {
    const block = m[1];
    const nameMatch = block.match(/<a[^>]*class="[^"]*company-name[^"]*"[^>]*>([^<]+)<\/a>/i)
                   || block.match(/<h[23][^>]*>\s*<a[^>]*>([^<]+)<\/a>/i);
    const addrMatch = block.match(/<span[^>]*class="[^"]*address[^"]*"[^>]*>([^<]+)<\/span>/i)
                   || block.match(/<div[^>]*class="[^"]*address[^"]*"[^>]*>([^<]+)<\/div>/i);
    const phoneMatch = block.match(/<span[^>]*class="[^"]*phone[^"]*"[^>]*>([^<]+)<\/span>/i)
                    || block.match(/tel[^"]*">([0-9\s+()-]+)</i);
    const linkMatch = block.match(/<a[^>]*href="(https:\/\/www\.cylex-uk\.co\.uk\/[^"]+)"/i);
...
export async function checkPresence(canonicalNap) {
  ...
  try {
    ...
    const url = `https://www.cylex-uk.co.uk/search/${query}-${location}.html`;
