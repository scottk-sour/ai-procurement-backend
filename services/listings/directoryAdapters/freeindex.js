...
function parseResults(html) {
  const results = [];
  const pattern = /<div[^>]*class="[^"]*listing_results[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/gi;
  let m;
  while ((m = pattern.exec(html)) !== null) {
    const block = m[1];
    const nameMatch = block.match(/<a[^>]*class="[^"]*listing_name[^"]*"[^>]*>([^<]+)<\/a>/i)
                   || block.match(/<h[23][^>]*>\s*<a[^>]*>([^<]+)<\/a>/i);
    const addrMatch = block.match(/<span[^>]*class="[^"]*address[^"]*"[^>]*>([^<]+)<\/span>/i)
                   || block.match(/<p[^>]*class="[^"]*location[^"]*"[^>]*>([^<]+)<\/p>/i);
    const phoneMatch = block.match(/<span[^>]*class="[^"]*phone[^"]*"[^>]*>([^<]+)<\/span>/i)
                    || block.match(/tel[^"]*">([0-9\s+()-]+)</i);
    const linkMatch = block.match(/<a[^>]*href="(\/profile\/[^"]+)"/i);
    ...
    if (nameMatch) {
      results.push({
        ...
        postcode: '',
        url: linkMatch ? `https://www.freeindex.co.uk${linkMatch[1]}` : null,
      });
    }
  }
...
export async function checkPresence(canonicalNap) {
  ...
  try {
    ...
    const url = `https://www.freeindex.co.uk/searchresults.htm?k=${query}&l=${location}`;
