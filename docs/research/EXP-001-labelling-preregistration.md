...
28/08/2026 — Adjudication method. The 267 verdicts were drafted by an AI adjudicator from the blinded labelling view and accepted by Scott unchanged, rather than hand-labelled by a single human labeller as specified in §5–§7. Blinding was also not preserved on the adjudicator's side: contrary to §5 (which requires the key to stay closed until every verdict is recorded), the labelling key was present in the adjudicator's session alongside the view. Both departures are recorded here and in the scoring artefact docs/research/EXP-001-rule-scoring-2026-08-28.md.
01/09/2026 — Scoring artefact committed. The scoring artefact referenced in the 28/08/2026 entry above now exists in this repository at `docs/research/EXP-001-rule-scoring-2026-08-28.md`; that reference is no longer forward-looking. The artefact holds the raw `scoreCandidateRules.js` stdout from commit `f6d64b6`, including the per-row verdict table for all 267 rows — the only record of the 28/08/2026 adjudication held in this repository, since `buildLabellingView.js` regenerates the labelling view with `human_verdict` empty by design. It was not committed on 28/08/2026 because the paste of the report body did not complete.
01/09/2026 — Second scoring artefact (six rules). The same 267 verdicts have been scored again against six candidate rules, and the result committed at `docs/research/EXP-001-rule-scoring-six-rules-2026-09-01.md`. The 28/08/2026 artefact scored three rules; the three retained-suffix rules were added to `scoreCandidateRules.js` in `c374b9a` and reached `main` in `5ce3043`. The verdicts are unchanged and were restored into the regenerated labelling view by `scripts/experiments/restoreVerdicts.js` from the 28/08/2026 artefact; the two artefacts' per-row tables agree on all 267 rows. The deviations recorded in the 28/08/2026 entry above apply to the second artefact in full — scoring more rules against the same labels does not improve the labels.
02/09/2026 — Regression-gate target is 29, not 30. Rule selection used RETAINED-SUFFIX applied *uniformly* (30 total errors at the pre-registered lower bound, AMBIG=FALSE). The rule actually deployed — call it RETAINED-SUFFIX-SINGLE — applies the retained form on the single-token path only and keeps `includes(normFirm)` on the multi-token path (Hard Rule 7 of PR #186). The two diverge on 7 multi-token rows (`row_uid` 31, 78, 146, 155, 160, 164, 217); the deployed rule scores 29 at AMBIG=FALSE and 27 at AMBIG=REAL — one and five errors better than the uniform variant — because `includes()` finds multi-token REAL firms (31, 78, 146, 155) that the retained-phrase match misses. The regression gate's expected `CURRENT` total is therefore 29 (lower) / 27 (upper), not 30. This was anticipated in the PR #186 description; the full run is recorded in `docs/research/EXP-001-regression-2026-09-02.md`. Gate run at commit `f90a3ca`. Decision: accept the deployed rule as-is; the live matcher and the scorer's rule definitions are unchanged.
