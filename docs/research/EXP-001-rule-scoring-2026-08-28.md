...
Scoring artefact for `study_2026_07_exp001`. This is the record referenced by
`docs/research/EXP-001-labelling-preregistration.md` §13 (28/08/2026 entry).
...
| Adjudication date | 28/08/2026 |
| Scoring commit | `f6d64b6` — merge of PR #176 (`feat/exp-001-rule-scorer`), 28 Aug 2026 17:27 +0100 |
| Rules scored | CURRENT, BOUNDARY-SINGLE, BOUNDARY-BOTH |
| Report body | Raw `scripts/experiments/scoreCandidateRules.js` stdout, verbatim and unreformatted |

Three rules, not six. The three retained-suffix rules (RETAINED-SUFFIX,
BOUNDARY-PLUS-RETAINED, RETAINED-THEN-BARE) were added to the scorer later, in
`c374b9a` on 01/09/2026, and are therefore absent from this run. At `f6d64b6` the
scorer defined exactly the three rules named above.
...
All three inputs are gitignored and are not held in this repository. They
regenerate deterministically from the seeds above. `buildLabellingView.js` writes
`human_verdict` empty for every row by design, and enforces it (integrity check 9:
*"human_verdict empty for every view row"*), so a regenerated view never carries
the verdicts. That is why this artefact matters: the per-row table below is the
record of them.

## Deviation from the pre-registration

Recorded here as it is recorded at §13, without softening.
...
This is not the procedure §5–§7 specifies. Both departures bear directly on how
much weight the figures below can carry, and neither is cured by the verification
recorded next.

## Verification

Two checks, recorded separately because they rest on different evidence.

**1. Three-way comparison — performed by Scott, outside this repository.** A
recovered filled `labelling-view.csv`, a recovered `labelling-verdicts-ai-draft.csv`,
and the per-row table in this artefact were compared across all 267 rows: identical
`row_uid` sets, every verdict matching, zero disagreements. Neither recovered file
is held in this repository, so this comparison is recorded **as reported** and has
not been independently reproduced here.

**2. Internal consistency — reproduced from this artefact alone.** Recounting the
per-row table reproduces every summary figure the report states:
...
- Single-token (n=242): REAL 112 / FALSE 125 / AMBIGUOUS 5
- Multi-token (n=25): REAL 19 / FALSE 1 / AMBIGUOUS 5
- `row_uid` values are contiguous 1–267, with no duplicates and no gaps
- All six residual-disagreement `row_uid` lists match the table element for element
- Exactly one row (`row_uid` 217) has BOUNDARY-SINGLE ≠ BOUNDARY-BOTH, and it is
  the row marked `*`

The body is also consistent with the scorer as it stood at `f6d64b6`: that revision
defines exactly the three rules named, and its per-row printer emits the column
layout used below.

## Report body

Raw `scoreCandidateRules.js` stdout at `f6d64b6`, verbatim — not reformatted,
re-sorted or edited in any way. The per-row table is the primary record of the
adjudication.
