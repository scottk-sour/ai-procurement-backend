/**
 * mentionMatcher.pre186.js — FROZEN COPY of scripts/experiments/lib/mentionMatcher.js
 * as it stood at commit a080129, immediately before PR #186 replaced the
 * single-token path with RETAINED-SUFFIX.
 *
 * This is the CONTEXT-GATE matcher that DEFINED the 267-row EXP-001 fixture
 * (Stratum A = current-vs-substring disagreement; Stratum B = current positives).
 * It is frozen here for FIXTURE REPRODUCTION ONLY, so the fixture's membership can
 * be rebuilt on the Render box — which clones shallow with no remote and therefore
 * has no git history to recover this revision from.
 *
 * MUST NEVER be used for scoring or recomputation. The live production matcher is
 * scripts/experiments/lib/mentionMatcher.js; use that for everything except
 * regenerating the frozen fixture's membership.
 *
 * The code below is a verbatim copy retrieved from git history (a080129).
